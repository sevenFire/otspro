...
public class AuthorizationFilter implements ContainerRequestFilter {
    ...
    public ContainerRequest filter(ContainerRequest request) {        
    	...
    	if(method.equals(CommonConstants.AUTH_CROSSDOMAINN_CHECK)) {
      	...
      	// check rest API
    	//!!! No auth for _tenant_init temporarily !!!
    	if (absolutePath.contains(CommonConstants.DEFAULT_API_PATH) && !absolutePath.contains(CommonConstants.DEFAULT_API_INTENAMTINIT_PATH)) { 
    		...
    		if (token != null && !absolutePath.contains(CommonConstants.DEFAULT_API_AUTH_TOKEN_PATH)) {
	    		...
	    		if (authentication != null) {		    	
			        ...
			        if (tokenManager.check(model)) {
			            ...
			            if (absolutePath.contains(CommonConstants.DEFAULT_API_AUTH_TOKEN_PATH)) {
				            webRequest.setAttribute(CommonConstants.IN_SESSION_PASSWORD_KEY, password);//Password	
						}
